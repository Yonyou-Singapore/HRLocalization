...
public class ItemBillFormEditor extends HrBillFormEditor {
...
//        billModel.addDecimalListener(new AbstractBillItemRowDigitsAdapter(billModel,WaItemVO.IFLDDECIMAL, new String[]{
//        		WaItemVO.NSUMCEIL, WaItemVO.NSUMFLOOR,WaItemVO.NPSNCEIL, WaItemVO.NPSNFLOOR})
//        {
//            @Override
...
//		new CheckedTextField("psnfloor","psnfloorflag","npsnfloor"),
	...
	/*
	 * 选择是否为汇总项目后的调整UI可编辑字段的私有方法
	 */
	private void adjustEditableForIsTotalItem(Boolean isTotalItem) {
		ItemDataSourcePanel dataSourcePanel = getItemDataSourcePanel();
		DataSourceItem box = dataSourcePanel.getCurrentDataSourceItem();
		WaItemVO itemVO = (WaItemVO) getValue();
		if (isTotalItem == true) {	// 清空公式内容并将数据来源 公式内容全部清空
			itemVO.setIfromflag(FromEnumVO.FORMULA.value());
			if (itemVO.getVformula() == null || itemVO.getVformula().equals("")) {
				itemVO.setVformula("0");
			}
			if (itemVO.getVformulastr() == null || itemVO.getVformula().equals("")) {
				itemVO.setVformulastr("0");
			}
			dataSourcePanel.setDataToEditor(itemVO);
			dataSourcePanel.setContentEnabled(false);
		} else {
			dataSourcePanel.setContentEnabled(true);
		}
	}
	...
	public void afterEdit(BillEditEvent e) {
		if(WaItemVO.IITEMTYPE.equals(e.getKey())){
			...
			if(PropertyEnumVO.SYSTEM.value().equals(itype)  ){
				...
				//				MessageDialog.showErrorDlg(ItemBillFormEditor.this, null, ResHelper.getString("60130glbitem","060130glbitem0031")/*@res "不能新增系统项目"*/);
				//				throw new BusinessException(ResHelper.getString("60130glbitem","060130glbitem0031")/*@res "不能新增系统项目"*/);
				ShowStatusBarMsgUtil.showErrorMsg(ResHelper.getString("6013commonbasic","06013commonbasic0020")/*@res "错误"*/,
						ResHelper.getString("60130glbitem","060130glbitem0031")/*@res "不能新增系统项目"*/, getModel().getContext());
				return;
			}
			...
			//				MessageDialog.showHintDlg(ItemBillFormEditor.this, ResHelper.getString("60130glbitem","060130glbitem0032")/*@res "提醒"*/, ResHelper.getString("60130glbitem","060130glbitem0033")/*@res "数据类型发生变化，公式需要重新定义，否则将无法保存公式定义的信息!"*/);
			//			}
		...
		} else if (WaItemVO.G_ISTOTALITEM.equals(e.getKey())){ // 当选择了是否为汇总项后的逻辑
			Boolean isTotalItem = (Boolean) getHeadItem(WaItemVO.G_ISTOTALITEM).getValueObject();
			
			adjustEditableForIsTotalItem(isTotalItem);
		}
	}
...
//		resetPrecision();
	...
	/*
	private void setFldwidthMinValue(){
		   BillItem fldwidth  = getHeadItem(WaItemVO.IFLDWIDTH);
		   Integer ifldwidth = (Integer) fldwidth.getValueObject();
		   if(ifldwidth==null){
			   ifldwidth = 12;
	...
	private void setFlddecimalMinValue(){
		BillItem flddecimal  = getHeadItem(WaItemVO.IFLDDECIMAL);
		   Integer iflddecimal = (Integer) flddecimal.getValueObject();
		   if(iflddecimal==null){
			   iflddecimal = 2;
