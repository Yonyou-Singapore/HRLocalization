...
import nc.ui.wa.paydata.view.dialog.OnTimePayDlg;
...
//			if (showYesNoMessage(ResHelper.getString("60130paydata","060130paydata0332")/*@res "有员工调薪，应先进行计算,是否继续?"*/) != MessageDialog.ID_YES) {
			//自动弹出时点薪资的对话框 add by weiningc 20191011  start
			OnTimePayDlg ontimedlg = new OnTimePayDlg(getParentContainer(), getWaLoginVO());
			ontimedlg.showModal();
			if(UIDialog.ID_OK != ontimedlg.getResult()) {
				return;
			}
			//refresh
			getPaydataManager().refresh();
//			return;
			//end
//			}
		}
