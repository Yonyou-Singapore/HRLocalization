...
public class WadataTemplateContainer extends nc.ui.uif2.editor.TemplateContainer implements IPaydataModel, ICommonAlterName
{
    ...
    private static Map<String, DefdocVO> TABS_SALARY = null;
    ...
    private BillTempletHeadVO getBillTempletHeadVO()
    {
        ...
        //add chenth 20190522 卡片编辑支持分类展示
        Map<String, DefdocVO> tabs = getTABS_SALARY();
        BillTabVO[] tabvos = new BillTabVO[4 + tabs.size()];
        int tabindex = 0;
        //add end 
        ...
        billTabVO.setTabindex(tabindex);
        //add chenth 20190522 卡片编辑支持分类展示
        tabvos[tabindex] = billTabVO;
        tabindex++;
        //add end 
        ...
        billTabVO1.setTabindex(tabindex);
        //add chenth 20190522 卡片编辑支持分类展示
        tabvos[tabindex] = billTabVO1;
        tabindex++;
        //add end 
        
        //add chenth 20190522 卡片编辑支持分类展示
        Iterator<Entry<String, DefdocVO>> iterator= tabs.entrySet().iterator();
        
        while(iterator.hasNext())
        {
        	Entry<String, DefdocVO> entry = iterator.next();
        	DefdocVO tabvo = entry.getValue();
        	BillTabVO billTabVOx = new BillTabVO();
            billTabVOx.setPos(IBillItem.HEAD);
            billTabVOx.setBasetab(TAB_NAME_KEY);
            billTabVOx.setTabcode(tabvo.getCode());
            billTabVOx.setTabname(tabvo.getName());
            billTabVOx.setTabindex(tabindex);
            tabvos[tabindex] = billTabVOx;
            tabindex ++;
        }
        //add chenth 
        ...
        billTabVO2.setTabindex(tabindex);
        //add chenth 20190522 卡片编辑支持分类展示
        tabvos[tabindex] = billTabVO2;
        tabindex ++;
        //add end 
        ...
        billTabVO3.setTabindex(tabindex);
        //add chenth 20190522 卡片编辑支持分类展示
        tabvos[tabindex] = billTabVO3;
        tabindex ++;
        //add end 
        
        
        BillStructVO bsVO = new BillStructVO();
        bsVO.setBillTabVOs(tabvos);
    ...
    public BillTempletBodyVO convert2TempletBodyVO(WaClassItemVO item)
    {
        ...
        //add chenth 20190522 卡片编辑支持分类展示
        String tabcode = item.getDef7();
        if(tabcode != null){
        	DefdocVO vo = getTABS_SALARY().get(tabcode);
        	if(vo != null){
        		tabcode = vo.getCode();
        	}else{
        		tabcode = TAB_NAME_ITEM;
        	}
        }else{
        	tabcode = TAB_NAME_ITEM;
        }
        //add end 
        BillTempletBodyVO billTempletBodyVO = getbaseBodyVO(tabcode);
        ...
        if (item.getIfromflag().equals(FromEnumVO.USER_INPUT.value()))
        {
            ...
            billTempletBodyVO.setForeground(Integer.valueOf(nc.ui.bill.tools.ColorConstants.COLOR_BLUE));
        }
        else
        {
    ...
    //add chenth 20190522 卡片编辑支持分类展示
    public static Map<String, DefdocVO> getTABS_SALARY() {
    	if(TABS_SALARY == null){
    		TABS_SALARY = new LinkedHashMap<String, DefdocVO>();
    		List<DefdocVO> vos = null;
    		try {
    			vos = (List<DefdocVO>) NCLocator.getInstance()
    					.lookup(IUAPQueryBS.class).retrieveByClause(DefdocVO.class
    							, " pk_defdoclist in (select pk_defdoclist from bd_defdoclist where code = 'HR10') "
    							, new String[]{DefdocVO.PK_DEFDOC, DefdocVO.CODE, DefdocVO.NAME});
    		} catch (BusinessException e) {
    			//TODO
    		}
    		if(vos != null && vos.size() > 0){
    			Collections.sort(vos, new DefdocDocComparator());
    			for(DefdocVO vo : vos){
    				TABS_SALARY.put(vo.getPk_defdoc(), vo);
    			}
    		}
    	}
