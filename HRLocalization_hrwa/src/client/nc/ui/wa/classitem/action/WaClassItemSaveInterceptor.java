	...
	public boolean beforeDoAction(Action action, ActionEvent e) {
		...
		if(vo.getPk_wa_item()==null){
			try {
				if (isOrgNode() && !groupAllowAddItem()) {
					showErrorDlg(ResHelper.getString("60130payitem","060130payitem0199")/*@res "集团不允许下级增加公共薪资项目,必须选择公共薪资项目！"*/);
					return false;
				}
		...
		if(vo.getTypeEnumVO().equals(TypeEnumVO.FLOATTYPE)){
			if(vo.getRound_type()==null){
				showErrorDlg(ResHelper.getString("60130payitem","060130payitem0200")/*@res "数值型项目的舍位方式不能为空"*/);
				return false;
			}
...
//			    showErrorDlg(ResHelper.getString("60130payitem","060130payitem0201")/*@res "薪资项目的小数位数需要小于数据长度。"*/);
//				return false;
//			}
			try {
				WaItemVO waItem = NCLocator.getInstance().lookup(IItemQueryService.class).queryWaItemVOByPk(vo.getPk_wa_item());
				if (waItem!=null&& null!=vo.getIflddecimal()&&vo.getIflddecimal().intValue() > waItem.getIflddecimal().intValue()) {
				    showErrorDlg(ResHelper.getString("60130payitem","060130payitem0202")/*@res "薪资发放项目的小数位数不能大于公共薪资项目的小数位数。"*/);
					return false;
				}
			} catch (BusinessException e1) {
				showErrorDlg(ResHelper.getString("60130payitem","060130payitem0203")/*@res "没有找到对应的公共薪资项目。"*/);
				return false;
			}
		}
		...
		if(StringUtil.isEmpty(formular)){
			if(vo.getFromEnumVO().equals(FromEnumVO.HI) ){				
					showErrorDlg(ResHelper.getString("60130payitem","060130payitem0205")/*@res "人事函数不能为空"*/);
					return false;
			}else if(vo.getFromEnumVO().equals(FromEnumVO.WAORTHER)){
				    showErrorDlg(ResHelper.getString("60130payitem","060130payitem0206")/*@res "薪资函数不能为空"*/);
					return false;
				...
				return false;
					...
					showErrorDlg(ResHelper.getString("60130payitem","060130payitem0038")/*@res "考勤函数不能为空"*/);
					return false;
			}if(vo.getFromEnumVO().equals(FromEnumVO.WA_WAGEFORM) ){				
					showErrorDlg(ResHelper.getString("60130payitem","060130payitem0207")/*@res "薪资规则表不能为空"*/);
					return false;
			}else if(vo.getFromEnumVO().equals(FromEnumVO.FORMULA) && (!ItemUtils.isSpecialSystemItem(vo.getItemkey())) ){
					//来自公式，并且不是系统项目
					showErrorDlg(ResHelper.getString("60130payitem","060130payitem0204")/*@res "公式不能为空"*/);
					return false;
			}else  if(vo.getFromEnumVO().equals(FromEnumVO.WA_GRADE) ){				
					showErrorDlg(ResHelper.getString("60130payitem","060130payitem0210")/*@res "薪资标准表不能为空"*/);
					return false;
		...
		if(!StringUtil.isEmpty(formular)){
			if(vo.getFromEnumVO().equals(FromEnumVO.WA_WAGEFORM) ){{
				...
				if (itemDecimal < ruleDecimal) {
				    String msg = ResHelper.getString("60130payitem","060130payitem0208")/*@res "薪资项目的小数位数小于薪资规则表的小数位数，是否继续?"*/;
				    if (MessageDialog.showYesNoDlg(getContainer(), ResHelper.getString("60130payitem","060130payitem0209")/*@res "提示"*/, msg) == UIDialog.ID_NO) {
					  return false;
				    }
				}
			}			
		...
		if(needExpand(vo)){
			...
			try {
				if(!util.expandTable()){
					showErrorDlg(ResHelper.getString("60130payitem","060130payitem0211")/*@res "薪资发放表扩展失败"*/);
					return false;
				}
	...
	private boolean groupAllowAddItem() throws BusinessException {
		...
		//allowed = NCLocator.getInstance().lookup(IHrPara.class).getBooleanValue(
		//		getModel().getContext().getPk_group(), ParaConstant.ADDITEM_PRVW, null, null);
