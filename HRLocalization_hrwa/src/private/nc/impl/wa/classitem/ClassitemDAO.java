...
public class ClassitemDAO extends BaseDAOManager{
	...
	public WaClassItemVO[] queryItemWithAccount(String pk_org, String pk_wa_class, String year, String period) throws DAOException {
		...
		Collection<WaClassItemVO> c = (Collection<WaClassItemVO>) new BaseDAO().executeQuery(sqlBuffer.toString(), param,
				new BeanListProcessor(WaClassItemVO.class));
		if (c == null){
			return null;
		}
	...
	public WaClassVO[]  subClassHasCheckedData(WaClassVO groupClassvo) throws BusinessException{
		...
		StringBuilder sbd =new StringBuilder();
		sbd.append(" select  wa_waclass.pk_wa_class ,  "+ SQLHelper.getMultiLangNameColumn("wa_waclass.name")+ " ,wa_waclass.name2 ,wa_waclass.name3 ,wa_waclass.code,wa_waclass.cyear,wa_waclass.cperiod  " +
	...
	public WaClassVO[] queryGroupAssignedWaclass(WaClassVO vo)
			throws DAOException {
		String sql = " select wa_waclass.*  "
				+ "from  wa_waclass,wa_assigncls "
				+ "where wa_waclass.pk_wa_class =  wa_assigncls.classid "
				+ "and  wa_assigncls.pk_sourcecls =? "
...
//				end
	...
	public WaClassItemVO  queryClassItemVO(String pk_wa_item,String cyear, String cperiod,String pk_wa_class) throws DAOException{
	...
	/***************************************************************************
	 * WaClassItemVO sql
	 * <br>Created on 2012-7-5 8:54:09<br>
	 * @return
	 * @author daicy
	 ***************************************************************************/
...
//				+ "wa_classitem.name ,wa_classitem.name2 ,wa_classitem.name3 ,wa_classitem.name4 ,wa_classitem.name5 ,wa_classitem.name6 ,wa_classitem.npsnceil,wa_classitem.npsnfloor,wa_classitem.nsumceil,wa_classitem.nsumfloor,wa_classitem.pk_group,wa_classitem.pk_org,wa_classitem.pk_wa_class,wa_classitem.pk_wa_classitem,wa_classitem.pk_wa_item,wa_classitem.psnceilflag,wa_classitem.psnfloorflag,wa_classitem.showflag,wa_classitem.sumceilflag,wa_classitem.sumfloorflag,wa_classitem.taxflag,wa_classitem.ts,wa_classitem.vformula,wa_classitem.bankaccount,wa_classitem.inapproveditem,wa_classitem.creator,wa_classitem.creationtime,wa_classitem.modifier,wa_classitem.modifiedtime,wa_classitem.round_type,wa_classitem.category_id,wa_classitem.vformulastr  ";
		...
		String sql = " select wa_item.clearflag,wa_item.code,  wa_item.defaultflag, wa_classitem.iflddecimal, wa_item.ifldwidth, wa_item.iitemtype, wa_item.intotalitem, wa_item.iprivil, wa_item.iproperty, wa_item.isinhi, wa_item.mid, wa_item.pk_budget_item,  wa_item.totalitem, "+
				...
				+ " wa_classitem.def1, wa_classitem.def2, wa_classitem.def3, wa_classitem.def4, wa_classitem.def5, wa_classitem.def6, wa_classitem.def7, wa_classitem.def8, wa_classitem.def9, wa_classitem.def10, wa_classitem.def11, "
				...
				+ " wa_classitem.def32, wa_classitem.def33, wa_classitem.def34, wa_classitem.def35, wa_classitem.def36, wa_classitem.def37, wa_classitem.def38, wa_classitem.def39, wa_classitem.def40 ";
		return sql;
	// end
	}
	...
	public void updateItemCaculateSeu(WaClassItemVO[] classitemVOs) throws BusinessException {
		...
		try {
			...
			throw new nc.vo.pub.BusinessException(ResHelper.getString("60130classpower","060130classpower0165")/*@res "更新项目计算顺序失败"*/);
		} finally {
	...
	public WaClassItemVO[] queryTransferClsItem(WaClassItemVO vo ) throws DAOException {
		StringBuilder sbd = new StringBuilder();
		sbd.append("select ");
		sbd.append("  "+ SQLHelper.getMultiLangNameColumn("wa_classitem.name")+ " ,wa_classitem.name2 ,wa_classitem.name3 ,wa_classitem.destitempk,wa_classitem.destitemcol ");
		sbd.append(" from  wa_classitem  ");
	...
	/***************************************************************************
	 * 判断一个项目是否在统一方案其他期间存在
	 * <br>Created on 2012-10-23 下午3:35:03<br>
	 * @param waclassVO
	 * @param itemKey
	 * @return
	 * @throws BusinessException
	...
	public boolean isExistInOtherChildClass(String parentClassPK,String childClassPK ,WaClassItemVO vo) throws BusinessException {
		StringBuffer sqlB = new StringBuffer();
		...
		sqlB.append( "  and wa_classitem.cperiod = '"+ vo.getCperiod() +"' and wa_classitem.pk_wa_item = '"+ vo.getPk_wa_item() +"' ");
		sqlB.append( "  and wa_classitem.pk_wa_class =  wa_inludeclass.pk_childclass and wa_classitem.pk_wa_class <> '"+   childClassPK +"' ");

		return new BaseDAOManager().isValueExist(sqlB.toString());
	}

	/**
	 * 核查薪资发放项目，被那些自定义公式的项目依赖
	 * 自定义公式的有： 来源公式、来源规则表、来源其他数据源（薪资）
	 *
	 * 是否被薪资分摊引用。
	 * @param itemKey
	 * @param pk_wa_class
	 ...
	 * @throws BusinessException
	 */
	public boolean checkUsed(String itemKey, String pk_wa_class, String pk_wa_classitem, String cyear, String cperiod) throws BusinessException {
		...
		//
		//			WaClassItemVO[] list = new BaseDAOManager().executeQueryVOs(sqlB.toString(), parameter, WaClassItemVO.class);//
		//			////////////////////////(sqlB.toString(), parameter, new BeanListProcessor(ClassitemVO.class));
		...
		// 薪资分摊是否被引用
		String sql = " select 1 from wa_amobase,wa_amoscheme  where wa_amobase.pk_amoscheme  = wa_amoscheme.pk_amoscheme  and wa_amoscheme.classid = '" + pk_wa_class + "' and 	wa_amobase.pk_wa_item = '"+ pk_wa_classitem +"'";
		Boolean isUserd = new BaseDAOManager().isValueExist(sql.toString());
		if (isUserd) {
			throw new BusinessException( ResHelper.getString("60130classpower","060130classpower0166")/*@res "薪资项目被薪资分摊引用！"*/);
		}
		...
		if (isUserd) {
			throw new BusinessException( ResHelper.getString("60130classpower","060130classpower0167")/*@res "被薪资项目结转引用！"*/);
		}
	...
	public void deleteWaclassItem(WaClassItemVO vo ) throws DAOException{
		...
		//删除该项目
		String sql = " delete from wa_classitem where pk_wa_class = '"+ vo.getPk_wa_class() +"' and cyear = '"+vo.getCyear()+"' and cperiod = '"+vo.getCperiod()+"' and pk_wa_item = '"+vo.getPk_wa_item()+"' "  ;
		getBaseDao().executeUpdate(sql);
	}

	//	public void updateWaclassItem(WaClassItemVO vo,String[] attributeNames) throws DAOException{
	//		//清空该项对应的数据
	//
	//		//删除该项目
	//		StringBuilder sbd = new StringBuilder();
	...
	//
	//		}
	//		String sql = " update wa_classitem  set "+ sbd.toString()+"  where pk_wa_class = '"+ vo.getPk_wa_class() +"' and cyear = '"+vo.getCyear()+"' and cperiod = '"+vo.getCperiod()+"' and pk_wa_item = '"+vo.getPk_wa_item()+"' "  ;
	//	    getBaseDao().executeUpdate(sql, para);
	//
	//	}


	//	public void updateParentItem(WaClassItemVO parentClassItem,WaClassItemVO childClassItem,String[] attributeNames) throws DAOException{
	//		//清空该项对应的数据
	//
	//		StringBuilder sbd = new StringBuilder();
	//		for (int i = 0; i < attributeNames.length; i++) {
	...
	public WaClassItemVO[] queryByCondition(LoginContext context, String strFromCond,String strWhereCond,String strOrderCond) throws BusinessException {
		String sql = getFileds();
		StringBuilder sbSQL = new StringBuilder(sql).append(" from wa_classitem inner join wa_item  on wa_item.pk_wa_item = wa_classitem.pk_wa_item ");
	...
	public WaClassItemVO[] queryItemsByPK_wa_class(String pk_wa_class, String cyear, String cperiod)
			throws DAOException {
		WaClassItemVO[] itemvos = this.retrieveByClause(WaClassItemVO.class, " pk_wa_class = '"+pk_wa_class+"' and cyear = '" + cyear + "' and cperiod = '" + cperiod + "' ");
		return itemvos;
	}
	
	//shenliangc 20140823 本方案本期间内薪资发放项目名称已存在
	public boolean itemNameIsExist(String pk_wa_class, String cyear, String cperiod, WaClassItemVO vo) throws DAOException{
		String sql = "select name from wa_classitem where " +
				" pk_wa_class = '" + pk_wa_class + "' and " +
				" cyear = '" + cyear + "' and " +
				" cperiod = '" + cperiod + "' and " +
				" name = '" + vo.getName() + "'" +
