 ...
 * @修改日期:
 */
public class ItemServiceImpl implements IItemManageService, IItemQueryService,Serializable
{
	...
	@Override
	public WaItemVO[] queryWaItemVOsByCondition(LoginContext context, String condition) throws BusinessException
	{
		WaItemVO[] vos = getServiceTemplate().queryByCondition(context, WaItemVO.class, condition, "  idisplayseq ,code ");
	 ...
	 */
	@Override
	public WaItemVO[] queryByConditionWithDispOrder(LoginContext context, String condition)
			throws BusinessException{
		...
		try
		{
			visibleWhere = VisibleUtil.getVisibleCondition(context, WaItemVO.class);
		}
		catch (Exception e)
		{
			Logger.error(e);
		...
		sbd1.append(" where " + visibleWhere.replaceAll("pk_org", "wa_item.pk_org") );
		...
		sbd2.append(" 	   ) ");
		sbd2.append(" and  " + visibleWhere.replaceAll("pk_org", "wa_item.pk_org") );
		...
		WaItemVO[] vos1 = new  BaseDAOManager().executeQueryVOs(sbd1.toString(), parameter, WaItemVO.class);
		WaItemVO[] vos2 = null;
		if(vos1==null&&context.getNodeType()==NODE_TYPE.ORG_NODE){
			...
			parameter.addParam(context.getPk_group());
			parameter.addParam(context.getPk_loginUser());
			vos1 = new  BaseDAOManager().executeQueryVOs(sbd1.toString(), parameter, WaItemVO.class);
			vos2 = new  BaseDAOManager().executeQueryVOs(sbd2.toString(), parameter, WaItemVO.class);
		}else{
			parameter = new SQLParameter();
			parameter.addParam(context.getPk_group());
			parameter.addParam(context.getPk_org());
...
//			fillTotalItem(result[i]);
//		}
		//guopeng 修改公共薪资项目打开节点效率问题
	...
	public WaItemVO[] querySystemItems() throws BusinessException
	{
		...
		return fillTotalItems(getServiceTemplate().queryByCondition(WaItemVO.class, where));
	}
	...
	public WaItemVO[] queryWaItemVOByPks(String[] pks) throws BusinessException
	{
		...
		try
		{
			...
			Collection<WaItemVO> items = new BaseDAO().retrieveByClause(WaItemVO.class,condition , "pk_wa_item");
	...
	@Override
	public WaItemVO[] queryWaItemVOForWadoc(LoginContext context) throws BusinessException
	{
		...
		{
			//guoqt HRWA007	定调资管理的业务处理组织如果是人事组织，但人员不归该人力资源组织管理，所以查询定调资显示项目的时候需要加过滤
			...
			sbSql.append(VisibleUtil.getVisibleCondition(context, WaItemVO.class));
			...
			sbSql.append(" and hi_psndoc_wadoc.pk_psnjob in "+ManagescopeFacade.queryPsnjobPksSQLByHrorgAndBusiregion(context.getPk_org(),busiregionEnum,true)+")");
		}
		...
		return fillTotalItems(getServiceTemplate().queryByPks(WaItemVO.class, pks));
	}
	...
	public void deleteWaItemVO(WaItemVO vo) throws BusinessException {
		...
		deleteSQL.append(" delete from hrp_budget_item_b where  pk_itemid = '" + vo.getPk_wa_item() + "' and (pk_budget_item in(select pk_budget_item from HRP_BUDGET_ITEM where PK_GROUP = '" + PubEnv.getPk_group() + "'))");
		dao.executeUpdate(deleteSQL.toString());

		//删除已分配的权限
		dao.executeUpdate(" delete from wa_itempower where pk_wa_item = '" + vo.getPk_wa_item() + "'");

		// 删除公共薪资项目
		getServiceTemplate().delete(vo);
		
		EventDispatcher.fireEvent(new BusinessEvent(HRWACommonConstants.WAITEMSOURCEID, IEventType.TYPE_DELETE_AFTER, vo));
		
		CacheProxy.fireDataDeleted(vo.getTableName(), vo.getPk_wa_item());
		CacheProxy.fireDataDeletedByWhereClause(BudgetSubVO.TABLE_NAME, BudgetSubVO.PK_BUDGET_ITEM_B,"pk_wa_item = '" + vo.getPk_wa_item() + "'");

	}

	@Override
	public 	WaItemDefdocCompseVO  getWaItemDefdocCompseVO(String pk_defdoclist,LoginContext context ,String wheresql) throws BusinessException{

		DefdoclistVO vo =  NCLocator.getInstance().lookup(IDefdoclistQryService.class).queryDefdoclistVOByPk(pk_defdoclist);

		DefdocVO[] vos =  NCLocator.getInstance().lookup(IDefdocQryService.class).queryDefdocVOsByDoclistPkAndWhereSql(pk_defdoclist, context.getPk_org(), context.getPk_group(), wheresql);
	...
	public WaItemVO insertWaItemVO(WaItemVO vo) throws BusinessException
	{
		...
		{
			throw new BusinessException(ResHelper.getString("60130glbitem","060130glbitem0034")/*@res "数据类型不能为空"*/);
		}
		if (itemId == null)
		{
			// 对项目进行扩展
			// NCLocator.getInstance().lookup(clazz)
			throw new BusinessException(ResHelper.getString("60130glbitem","060130glbitem0035")/*@res "该类型的项目已达到最多数不能再添加"*/);
		}
		...
		EventDispatcher.fireEvent(new BusinessEvent(HRWACommonConstants.WAITEMSOURCEID, IEventType.TYPE_INSERT_AFTER, vo));
	...
	private WaItemVO[] fillTotalItems(WaItemVO[] itemVOs) throws BusinessException
	{
		if (ArrayUtils.isEmpty(itemVOs))
		{
			return itemVOs;
		}
		//20150807 shenliangc 定调资信息维护-导出模板 数据库连接数优化 begin
		return batchFillTotalItem(itemVOs);
		
//		for (int i = 0; i < itemVOs.length; i++)
//		{
//			itemVOs[i] = fillTotalItem(itemVOs[i]);
//		}
//		return itemVOs;
		//20150807 shenliangc 定调资信息维护-导出模板 数据库连接数优化 end
	}
	
	//20150807 shenliangc 定调资信息维护-导出模板 数据库连接数优化
	private WaItemVO[] batchFillTotalItem(WaItemVO[] itemVOs) throws BusinessException{
		ArrayList<String> pkList = new ArrayList<String>();
		HashMap<String, ArrayList<String>> map = new HashMap<String, ArrayList<String>>();
		for(WaItemVO itemVO : itemVOs){
			pkList.add(itemVO.getPk_wa_item());
			map.put(itemVO.getPk_wa_item(), new ArrayList<String>());
		}
		String inSql = new InSQLCreator().getInSQL(pkList.toArray(new String[0]));
		// 查询总额项目
		StringBuffer sbSql = new StringBuffer();
		sbSql.append("  select ");
		...
		sbSql.append(" 	hrp_budget_item.pk_group = '" + PubEnv.getPk_group()
				+ "'");
		BudgetSubVO[] vos = new BaseDAOManager().executeQueryVOs(sbSql.toString(), BudgetSubVO.class);
		if (!ArrayUtils.isEmpty(vos)){
			for(BudgetSubVO vo : vos){
				map.get(vo.getPk_itemid()).add(vo.getPk_budget_item());
			}
		}
		// 总额项目使用格式 pk,pk1,pk2
		for(WaItemVO itemVO : itemVOs){
			ArrayList<String> budgetpks = map.get(itemVO.getPk_wa_item());
	...
	private WaItemVO fillTotalItem(WaItemVO itemVO) throws BusinessException
	{
		...
		sbSql.append(" 	on hrp_budget_item_b.pk_budget_item = hrp_budget_item.pk_budget_item ");
		sbSql.append("  where ");
		sbSql.append(" 	hrp_budget_item_b.pk_itemid = '" + itemVO.getPk_wa_item() + "' and ");
		sbSql.append(" 	hrp_budget_item.pk_group = '" + PubEnv.getPk_group()
				+ "'");
		BudgetSubVO[] vos = new BaseDAOManager().executeQueryVOs(sbSql.toString(), BudgetSubVO.class);
...
//		String sql = "select isnull(max(cast(substring(itemkey,3,len(itemkey)) as integer))+1,1) maxid from wa_item " + " where iitemtype = ?   and " + condition + " having  isnull(max(cast(substring(itemkey,3,len(itemkey)) as integer))+1,1) <= (select lmaxno from "+FreefldVO.getDefaultTableName()+" where iproducttype = 0 and ifieldtype = ?)";
		String sql = "";
		if(iType == 0) {//海外本地话字段不会更新最大薪资项目号, 普通手动薪资薪资项目过滤掉本地化字段 暂定为1000 by weiningc 20200303 start 
			sql = "select isnull(max(cast(substring(itemkey,3,len(itemkey)) as integer))+1,1) maxid from wa_item " + " where iitemtype = ?   and " + condition + " " 
				+ " and cast(substr(itemkey, 3, length(itemkey)) as integer) < 1000" 
				+ " having isnull(max(cast(substring(itemkey,3,len(itemkey)) as integer))+1,1) <= (select lmaxno from "+FreefldVO.getDefaultTableName()+" where iproducttype = 0 and ifieldtype = ?)";
		} else {
			sql = "select isnull(max(cast(substring(itemkey,3,len(itemkey)) as integer))+1,1) maxid from wa_item " + " where iitemtype = ?   and " + condition + " having  isnull(max(cast(substring(itemkey,3,len(itemkey)) as integer))+1,1) <= (select lmaxno from "+FreefldVO.getDefaultTableName()+" where iproducttype = 0 and ifieldtype = ?)";
		} 
		//end
		BaseDAO dao = new BaseDAO();
	...
	public void initItemByPsncl(LoginContext context, ArrayList<ItemVO> itemList) throws BusinessException
	{
		...
		ItemVO psnclItem = new ItemVO(NCLangResOnserver.getInstance().getStrByID("common", "UC000-0000140")/*
		 * @res
		 * "人员类别"
		 */);
	...
	public void initItemByDept(LoginContext context, ArrayList<ItemVO> itemList) throws BusinessException
	{
		...
		ItemVO deptItem = new ItemVO(NCLangResOnserver.getInstance().getStrByID("common", "UC000-0001975")/*
		 * @res
		 * "所在部门"
		 */);
	...
	@Override
	public WaItemVO[] queryWaItemVOsByGroup(String pk_group, String otherCondition) throws BusinessException
	{
		String where = "pk_group = '" + pk_group + "'" + (otherCondition == null ? "" : " and (" + otherCondition + " )");
		return fillTotalItems(getServiceTemplate().queryByCondition(WaItemVO.class, where));
	}

	@Override
	public WaItemVO[] queryWaItemVOsByOrg(String pk_group, String pk_org, String otherCondition) throws BusinessException
	{
		String where = " pk_group = '" + pk_group + "' and pk_org = '" + pk_org + "'" + (otherCondition == null ? "" : " and (" + otherCondition + " )");
		return fillTotalItems(getServiceTemplate().queryByCondition(WaItemVO.class, where));
	}
	 ...
	 */
	@Override
	public ItemVO[] getFormulaInitVO(LoginContext context) throws BusinessException
	{
	...
	public String querySelfDocPK(String code) throws BusinessException
	{
		...
		Collection<DefdoclistVO> c = baseDao.retrieveByClause(DefdoclistVO.class, " code = '" + code + "'");
		for (Iterator iterator = c.iterator(); iterator.hasNext();)
		{
			DefdoclistVO defdocVO = (DefdoclistVO) iterator.next();
			return defdocVO.getPk_defdoclist();
		}
		throw new BusinessException(ResHelper.getString("60130glbitem","060130glbitem0037")/*@res "没有找到自定义档案：薪资项目分类"*/);
	}

	@Override
	public WaItemVO queryByItemkey(String itemkey) throws BusinessException {
		BaseDAO baseDao = new BaseDAO();
		Collection<WaItemVO> c = baseDao.retrieveByClause(WaItemVO.class, " ITEMKEY = '" + itemkey + "'");
	...
	@Override
	public WaItemVO queryByItemkeyAndPkorg(String pk_org, String itemkey) throws BusinessException {
		BaseDAO baseDao = new BaseDAO();
		Collection<WaItemVO> c = baseDao.retrieveByClause(WaItemVO.class, " ITEMKEY = '" + itemkey + "' and pk_org = '" + pk_org + "' ");
		Iterator iterator = c.iterator();
		if(iterator.hasNext()){
			return (WaItemVO)iterator.next();
		}
	...
	@Override
	public WaItemVO queryByClassItemkeyAndPkorg(String pk_org, String itemkey) throws BusinessException {
		BaseDAO baseDao = new BaseDAO();
		Collection<WaItemVO> c = baseDao.retrieveByClause(WaClassItemVO.class, " ITEMKEY = '" + itemkey + "' and pk_org = '" + pk_org + "' ");
		Iterator iterator = c.iterator();
		if(iterator.hasNext()){
			return (WaItemVO)iterator.next();
		}
	...
	public WaItemVO queryByItemkeyAndPkorg(String itemkey) throws BusinessException {
		BaseDAO baseDao = new BaseDAO();
		Collection<WaItemVO> c = baseDao.retrieveByClause(WaItemVO.class, " ITEMKEY = '" + itemkey + "'");
		Iterator iterator = c.iterator();
		if(iterator.hasNext()){
	...
	@Override
	public WaItemVO[] queryByWaClass(String pk_prnt_class,String pk_wa_class) throws BusinessException{
		StringBuffer sqlStr = new StringBuffer();
		sqlStr.append("select distinct wa_item.category_id, wa_item.clearflag, wa_item.code, wa_item.creationtime, wa_item.creator, wa_item.defaultflag, " +
				...
				.append(" where wa_waclass.pk_wa_class='").append(pk_wa_class).append("'")
				.append("and wa_classitem.itemkey IN(SELECT wa_classitem.ITEMKEY ").append("   FROM wa_itempower inner join wa_classitem ON wa_classitem.pk_wa_item=wa_itempower.pk_wa_item and wa_classitem.PK_WA_CLASS=wa_itempower.PK_WA_CLASS ")
				.append("  WHERE wa_itempower.pk_wa_class ='").append(pk_prnt_class).append("'")
		...
		return new BaseDAOManager().executeQueryVOs(sqlStr.toString(), WaItemVO.class);

	}

	@Override
	public void saveItemDisplayOrder(LoginContext context,WaItemVO[] items) throws BusinessException {
	...
	public List<WaItemDspVO> queryItemDspVOs(LoginContext context)
		...
		params.addParam(context.getPk_org());
		params.addParam(context.getPk_loginUser());
		return (List<WaItemDspVO>)new BaseDAO().retrieveByClause(WaItemDspVO.class, condition, params);
	}
