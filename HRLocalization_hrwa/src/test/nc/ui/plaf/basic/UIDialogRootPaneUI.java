...
public class UIDialogRootPaneUI  extends BasicRootPaneUI 
{
	//dongdb+
    ...
    void installBorder(JRootPane root) {
        ...
        if (style == JRootPane.NONE) {
        	...
        	root.setBorder(getContentPaneBorder());
//            LookAndFeel.installBorder(root, borderKeys[style]);
    ...
    private JComponent createTitlePane(JRootPane root) {
    	if(isReset()==true){
    		return new UIDialogTitlePane(root,this,true);
    	}else{
    		 return new UIDialogTitlePane(root, this);
    	}
//        return new UIDialogTitlePane(root, this);
    }
    ...
    private class MetalRootLayout implements LayoutManager2 {
        ...
        public Dimension preferredLayoutSize(Container parent) {
            ...
            if (root.getWindowDecorationStyle() != JRootPane.NONE &&
                     (root.getUI() instanceof UIDialogRootPaneUI)) {
                JComponent titlePane = ((UIDialogRootPaneUI)root.getUI()).
                                       getTitlePane();
                if (titlePane != null) {
        ...
        public void layoutContainer(Container parent) {
            ...
            if(root.getMenuBar() != null) {
                ...
                root.getMenuBar().setBounds(3, nextY+1, w-6, mbd.height);
                nextY += mbd.height -4;
            }
            if(root.getContentPane() != null) {
//                Dimension cpd = root.getContentPane().getPreferredSize();
                root.getContentPane().setBounds(3, nextY+1, w-6, 
                h < nextY ? 0 : h - nextY-4);
            }
    ...
    private class MouseInputHandler implements MouseInputListener {
        ...
        public void mouseDragged(MouseEvent ev) {
            ...
            if (isMovingWindow) {
                ...
                int dx = eventLocationOnScreen.x - dragOffsetX;
                int dy = eventLocationOnScreen.y - dragOffsetY;
                
                Dimension screenSize = Toolkit.getDefaultToolkit().getScreenSize();
                
                Insets insets = Toolkit.getDefaultToolkit().getScreenInsets(GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice().getDefaultConfiguration());
                
                int titleHeight = getTitlePane().getHeight();
                
                int windowWidth = w.getWidth();
                
                int reserv= 10;
                dy = Math.min(dy,screenSize.height-insets.bottom-reserv);
                dy = Math.max(dy, insets.top-titleHeight+reserv);
                
                dx = Math.max(dx,insets.left-windowWidth+reserv);
                //这里取最大值 modify by weiningc 20190827 start
                int dx2 = Math.min(dx, screenSize.width-insets.right-reserv);
                dx = dx < dx2 ? dx2 : dx;
                //end
                ...
                if(w instanceof UIDialog){
                	((UIDialog)w).setLocationNoChange(dx, dy);
                }else{
                	w.setLocation(dx,dy);
                }
    ...
    public static final Color activeBorderColor1 = ThemeResourceCenter.getInstance().getColor("themeres/dialog/dialogResConf", "dialogContentPane.activeBorderColor1");
    public static final Color activeBorderColor2 = ThemeResourceCenter.getInstance().getColor("themeres/dialog/dialogResConf", "dialogContentPane.activeBorderColor2");
    public static final Color inactiveBorderColor1 = ThemeResourceCenter.getInstance().getColor("themeres/dialog/dialogResConf", "dialogContentPane.inactiveBorderColor1");
    public static final Color inactiveBorderColor2 = ThemeResourceCenter.getInstance().getColor("themeres/dialog/dialogResConf", "dialogContentPane.inactiveBorderColor2");
    public  class ContentPaneBorder extends AbstractBorder{
		private static final long serialVersionUID = 7136561361173462184L;
		private Insets insets = new Insets(1, 3,3,3);
		@Override
		...
		public void paintBorder(Component c, Graphics g, int x, int y, int w, int h) {
			if(getTitlePane() != null){
				Dimension size =getTitlePane().getSize();
				y += size.height;
				h -= size.height;
			}
			...
			Color color1 , color2;
			if(isActive){
				color1 = activeBorderColor1;
				color2 = activeBorderColor2;
			}else{
				color1 = inactiveBorderColor1;
				color2 = inactiveBorderColor2;
			}
			g.setColor(color1);
			...
			g.setColor(color2);
			g.drawLine(x+2, y, x+2, y+h-3);
			g.drawLine(x+2, y+h-3, x+w-3, y+h-3);
			g.drawLine(x+w-3, y, x+w-3, y+h-3);
			g.drawLine(x+2, y, x+w-3, y);
		}
    	
    }
    private  ContentPaneBorder contentPaneBorder = new ContentPaneBorder();
