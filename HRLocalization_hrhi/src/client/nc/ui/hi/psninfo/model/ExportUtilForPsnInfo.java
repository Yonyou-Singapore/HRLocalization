...
public class ExportUtilForPsnInfo {
	...
	private void setBodyData(String[] columNames,HSSFSheet sheet,GeneralVO[] genvos){
		//modify by weiningc 20181227 从第三行开始输出 start
		int notNullIndex = 2; //用来记录不为空的VO序号,并且将之作为输出索引，避免在excel表格中出现空行
		//end
...
//		String inSql = isc.getInSQL(pks);
	...
	private void setHeadNames(HSSFSheet sheet,String[] names,String[] infosetPks,String metaData) throws BusinessException{
		...
		codecell.setCellValue("hrhi.bd_psndoc.code");//这里需要字段名称
		namecell.setCellValue("hrhi.bd_psndoc.name");
		//add by weiningc 20181227 start
		this.creatCellName2body(row, names, metaData, metadataItemvoMap, "fullname");
		HSSFRow row2 = sheet.createRow(1);
		HSSFCell codecell2 = row2.createCell(0);
		HSSFCell namecell2 = row2.createCell(1);
		codecell2.setCellValue(ResHelper.getString("6007psn", "2UC000-000053")/*人员编码*/);
		namecell2.setCellValue(ResHelper.getString("6007psn", "UC000-0001403")/*姓名*/);
		this.creatCellName2body(row2, names, metaData, metadataItemvoMap, null);
	}
...
//		namecell.setCellValue(ResHelper.getString("6007psn", "UC000-0001403")/*姓名*/);
		this.creatCellName(row, names, metaData, metadataItemvoMap, "fullname");
		HSSFRow row2 = sheet.createRow(1); //第二行为具体字段名称 如编码等等
		this.creatCellName(row2, names, metaData, metadataItemvoMap, null);
	}
	
	/**
	 * nametype 表示生成的格式中会有fullname 即元数据 + 字段名   主表表头信息
	 * @param row
	 * @param names
	 * @param metaData
	 * @param metadataItemvoMap
	 * @param nametype
	 */
	private void creatCellName(HSSFRow row, String[] names, String metaData, HashMap<String,InfoItemVO> metadataItemvoMap,
			String nametype) {
		for(int i=1; i<names.length; i++){
			...
			//modify by weiningc 20181227 修改导出格式 导出格式自带字段名
			if("fullname".equals(nametype)) {
				if(metadataItemvoMap.get(metadata1)!=null) {
					cell.setCellValue(metadata1);
				} else {
					cell.setCellValue(metadata2);
				}
			} else {
	...
	/**
	 * nametype 表示生成的格式中会有fullname 即元数据 + 字段名   字表信息
	 * @param row
	 * @param names
	 * @param metaData
	 * @param metadataItemvoMap
	 * @param nametype
	 */
	private void creatCellName2body(HSSFRow row, String[] names, String metaData, HashMap<String,InfoItemVO> metadataItemvoMap,
			String nametype) {
