    ...
    IActionDriveChecker, ICheckStatusCallback
{
    ...
    public AggEntryapplyVO[] execBills(AggEntryapplyVO[] billvos) throws BusinessException
    {
        ...
        if (!StringUtils.isBlank(msg))
        {
            ...
            message[0] = ncMessage;
            //注释掉入职的时候发送消息 deleted by weiningc 20191007 start
//            try
//            {
...
//            }
    ...
    public AggEntryapplyVO[] doCommit(AggEntryapplyVO[] vos) throws BusinessException
    {
        ...
        if (approvetype != null && approvetype == HRConstEnum.APPROVE_TYPE_FORCE_DIRECT)
        {
            ...
            //deleted by weiningc 20191007 start
...
//            HiSendMsgHelper.sendMessage1(tempCode, hmTrans.get(key).toArray(new AggEntryapplyVO[0]), key);
     ...
     * 校验当前驱动是否允许
     * 
     * @param srcBilltype 来源单据类型PK
     * @param srcBillVo 源单据VO
     * @param srcAction 来源动作编码，比如"APPROVE"等
     * @param destBilltype 目的单据类型PK
     * @param drivedAction 被驱动动作编码，比如"WRITE"等
     * @return true允许；false不允许
     * @throws BusinessException
     */
    @Override
    public boolean isEnableDrive(String srcBilltype, AggregatedValueObject srcBillVO, String srcAction, String destBillType,
